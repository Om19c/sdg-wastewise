...
:root {
  --bg-color: #F0FDF4; /* Very soft mint background */
  --card-bg: #FFFFFF;
  --text-dark: #064E3B;
  --text-gray: #4B5563;
  ...
  --secondary: #FDE68A; /* Soft yellow for highlights */
...
.h-icon { font-size: 32px; background: white; width: 60px; height: 60px; display: flex; align-items: center; justify-content: center; border-radius: 16px; box-shadow: var(--shadow-sm); }
...
.tip-card { padding: 24px; border-radius: 24px; text-align: center; transition: transform 0.2s; }
.tip-card:hover { transform: translateY(-5px); }
.tip-emoji { font-size: 48px; margin-bottom: 16px; }
.tip-title { font-size: 18px; font-weight: 800; margin-bottom: 10px; }
  ...
  return (
    <div className="friendly-bar-container">
      <div className="friendly-bar-header">
        <span>{label}</span>
        <span style={{color: color}}>{value} kg</span>
      </div>
  ...
  return (
    <div className="fade-in">
      <div className="score-wrapper">
        <h2 style={{margin: '0 0 20px 0', fontSize: '24px', fontWeight: 800}}>Your Eco Score 🌟</h2>
      ...
      <div className="stats-grid">
        <div className="stat-box">
          ...
          <div className="stat-val">{totalWaste} <span style={{fontSize: '18px'}}>kg</span></div>
          <div className="stat-lbl">Food Wasted</div>
        </div>
        <div className="stat-box">
          <div className="stat-emoji">💸</div>
          <div className="stat-val">₹{costLoss}</div>
    ...
    <div className="fade-in">
      <div className="card" style={{border: '4px solid #D1FAE5'}}>
        ...
        <p style={{fontWeight: 600, marginBottom: '24px', color: 'var(--text-gray)'}}>
          Got some leftovers that can't be saved? Let's log it so we can learn to cook less tomorrow!
        </p>
        ...
        <form onSubmit={saveEntry}>
          ...
          <button type="submit" className="action-btn">
            <span>💾</span> Save to our records!
          </button>
          
          {showYay && (
            <div className="success-message">
              <span style={{fontSize: '24px'}}>🎉</span> Awesome! You're helping us get smarter about waste.
            </div>
          )}
        </form>
      </div>

      <h3 style={{fontSize: '20px', fontWeight: 800, margin: '32px 0 16px'}}>Recently Tracked 🕒</h3>
          ...
          <li key={log.id} className="history-card">
            <div className="h-info">
              ...
              <div>
                ...
                <div className="h-sub">From {log.mealSlot} on {log.date}</div>
              </div>
            </div>
            <button type="button" className="h-delete" onClick={() => setLogs(logs.filter(l => l.id !== log.id))} title="Remove entry">×</button>
          </li>
        ))}
        {logs.length === 0 && <p style={{fontWeight: 700, color: 'var(--text-gray)', textAlign: 'center', padding: '20px'}}>No waste logged recently!</p>}
      </ul>
    </div>
  );
};
...
const AiTipsView = () => {
  ...
  return (
    <div className="fade-in card">
      <div className="card-title" style={{fontSize: '28px', flexDirection: 'column', alignItems: 'center', gap: '16px'}}>
        <span style={{fontSize: '64px', animation: 'bounce 2s infinite'}}>💡</span>
        Smart Tips for our Canteen!
      </div>
      <p style={{textAlign: 'center', fontSize: '18px', fontWeight: 600, color: 'var(--text-gray)', marginBottom: '40px'}}>
        Our friendly AI bot looks at what we usually waste, and gives us super simple ideas on how to fix it!
      </p>

      {!showTips && !loading && (
        <div style={{display: 'flex', justifyContent: 'center'}}>
          <button className="action-btn" style={{maxWidth: '300px'}} onClick={getTips}>
            ✨ Show me the Magic Tips!
          </button>
        </div>
      )}
      ...
      {loading && (
        <div className="loader-wrapper">
          ...
          <h3 style={{margin: 0, color: 'var(--primary)', fontWeight: 800}}>Thinking of good ideas...</h3>
        </div>
      )}
      ...
      {showTips && (
        <div className="fade-in">
          <div className="tips-grid">
             <TipCard bg="#FEF3C7" icon="🍚" title="Cook less Rice for Lunch" desc="Rice is our #1 wasted item! Try making 2 bowls less than usual." />
             <TipCard bg="#D1FAE5" icon="🍎" title="Use Veggie Peels" desc="Don't throw away potato or carrot peels! We can give them to the campus compost." />
             <TipCard bg="#DBEAFE" icon="🥣" title="Save Dal for Tomorrow" desc="Put the leftover dinner dal in the fridge quickly so we can make tasty parathas for breakfast!" />
             <TipCard bg="#FCE7F3" icon="📝" title="Use Smaller Spoons" desc="If students use a slightly smaller spoon, they magically take only what they can eat!" />
          </div>
          
          <div style={{display: 'flex', justifyContent: 'center', marginTop: '40px'}}>
            <button className="pill-btn selected" onClick={() => setShowTips(false)}>Close Tips</button>
          </div>
        </div>
      )}
    </div>
...
const LearnView = () => (
  <div className="fade-in card">
    ...
    <p style={{fontWeight: 700, fontSize: '16px', color: 'var(--text-gray)', marginBottom: '32px'}}>
      If everyone helps out just a little bit, we can make our campus totally green and waste-free!
    </p>
...
const SurveyView = () => (
  <div className="fade-in">
    <div className="card" style={{border: '4px solid #FDE68A'}}>
      ...
      <p style={{fontWeight: 700, marginBottom: '24px'}}>We walked around campus and asked everyone about canteen food. Here is what we found:</p>
      
      <ul style={{fontSize: '16px', fontWeight: 700, color: 'var(--text-dark)', lineHeight: '2', paddingLeft: '20px', margin: 0}}>
        <li><span style={{fontSize: '20px'}}>❌</span> Nobody knows how much food we throw away every day!</li>
        <li><span style={{fontSize: '20px'}}>🚮</span> Food scraps and plastic wrappers are being thrown in the exact same dustbin.</li>
        <li><span style={{fontSize: '20px'}}>🥡</span> A lot of perfectly good food gets tossed because there's no system to share it.</li>
        <li><span style={{fontSize: '20px'}}>🍛</span> Rice and Sabzi are the things left on plates the most.</li>
        <li style={{color: 'var(--primary)'}}><span style={{fontSize: '20px'}}>✅</span> Good news: The canteen staff said they would love an easy app (like this one!) to help solve it.</li>
      </ul>
    </div>
  </div>
);
...
const ObjectiveView = () => (
  <div className="fade-in card" style={{textAlign: 'center', background: 'white', padding: '60px 40px'}}>
    ...
    <p style={{fontSize: '18px', fontWeight: 600, color: 'var(--text-gray)', maxWidth: '600px', margin: '0 auto 40px', lineHeight: 1.6}}>
      Our goal is to make our college canteen friendly, smart, and completely eco-friendly! 
      By simply pressing a few buttons on this app every day, we figure out how to stop throwing away good food and save the planet together!
    </p>
    ...
    <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px', textAlign: 'left'}}>
      <div style={{background: '#FEF3C7', padding: '24px', borderRadius: '24px'}}>
        ...
        <p style={{margin: 0, fontWeight: 600, fontSize: '14px', color: '#B45309'}}>Making sure fresh extra food goes to people, not the trash.</p>
      </div>
      ...
      <div style={{background: '#FFEDD5', padding: '24px', borderRadius: '24px'}}>
        ...
        <p style={{margin: 0, fontWeight: 600, fontSize: '14px', color: '#9A3412'}}>Buying, cooking, and eating only what we really need.</p>
      </div>
    ...
    <>
      ...
      <div className="app-container">
        ...
        <div className="header">
          ...
          <p className="subtitle">Let's make our canteen completely green!</p>
        </div>
